...
async function init() {
    try {
        ...
        const now = new Date();
        const nextIdx = allRaces.findIndex(r => new Date(r.sessions.gp) > now);
        const startIndex = nextIdx !== -1 ? nextIdx : 0;
        
        drp.value = startIndex;
        updateUI(startIndex);

        drp.addEventListener('change', (e) => updateUI(e.target.value));
    } catch (e) { console.error("Error loading race data", e); }
}
...
function updateUI(idx) {
    ...
    for (const [continent, cities] of Object.entries(continentData)) {
        ...
        cities.forEach(city => {
            ...
            let content = `<h4 style="margin:0 0 10px 0; border-bottom:2px solid #eee; padding-bottom:5px;">${city.name}</h4>`;
            for (const [s, t] of Object.entries(race.sessions)) {
                ...
                const dateStr = date.toLocaleDateString('en-GB', { timeZone: city.tz, weekday: 'short', day: '2-digit', month: 'short', year: 'numeric' });

                content += `
                    <div class="tooltip-row">
                        <div style="display:flex; flex-direction:column">
                            <span style="font-weight:bold; font-size:0.85rem;">${s.toUpperCase()}</span>
